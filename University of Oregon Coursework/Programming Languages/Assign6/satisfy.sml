...
fun satisfy (Var v, asn, k) =
  ...
  | satisfy (And(p1,p2), asn, k) = let
							 fun cont(env) = (let
										val a = satisfy(p1,env,k)	
										val b = satisfy(p2,env,k)
									in
										if (b = true andalso a = true) then true else false
									end)
						in 
							satisfy(p1,asn,cont)
						end

  | satisfy (Or(p1, p2), asn, k) = let
								fun cont(env) = (let
										val a = satisfy(p1,env,k)
										val b = satisfy(p2,env,k)
									in
										if (a = true orelse b = true) then true else false
									end)
								in 
									satisfy(p1,asn,cont)
								end
...
and falsify (Var v, asn, k) =
  ...
  | falsify (And(p1,p2), asn, k) = let
							 fun cont(env) = (let
										val a = falsify(p1,env,k)	
										val b = falsify(p2,env,k)
									in
										if (a = true orelse b = true) then true else false
									end)
						in 
  ...
  | falsify (Or(p1, p2), asn, k) = let
							 fun cont(env) = (let
										val a = falsify(p1,env,k)	
										val b = falsify(p2,env,k)
									in
										if (a = true andalso b = true) then true else false
									end)
						in 
							falsify(p1,asn,cont)
						end
